...
- `/about/team/`'s Board section was built out for real: three named board members (Daryl Spivey, Dr. Josh Fessel, Junelle Speller) each got a photo, role, and full bio via a 3D flip card (front = photo + name/role over a dark header band, back = full bio, floats with an ambient shadow). Resolves part of the "board bios beyond the founder" gap noted below. Still open: what each member specifically does for the org's governance (chair? clinical advisor?) — the bios describe their outside careers, not their board function.
- An independent full-site audit (design/UX/engineering/content/SEO/trust, scored per page) was run against the whole site and published as a report; fixes are being worked through it now. Two forms/embed swaps from that pass: **Formspree → Web3Forms** for Contact and Partner With Us (one shared `PUBLIC_WEB3FORMS_ACCESS_KEY` instead of Formspree's per-form IDs; `07-tech-stack-decision.md` and `12-analytics-conversion-tracking.md` still name Formspree and are now stale on this point). Also added: visible top-level desktop nav (previously hamburger-only at every width, including desktop), a Web3Forms honeypot field on both forms, disabled-button styling that reads as "off" rather than a faded active button, and `DiseasePage.astro` (a dead, unused scaffold component) deleted.
- **Flagged, not yet resolved:** the audit's Vision section proposed a Spanish-language track for FSGS/APOL1/The Need, backed by the Bronx population data already on the site (55.4% Hispanic). Ticket 01 explicitly lists "Multi-language support" under Out of scope for v1. This is a real conflict between a data-backed recommendation and a locked v1 boundary — needs a founder decision, not a silent override in either direction.
...
- Legal review of Privacy/Terms; clinical-accuracy review of the FSGS/APOL1 disease pages. (Board bios beyond the founder are now done — see Build phase.)
